describe("a user should be able to search by keyword", () => {
  ...
  it("should autosuggest commonly searched skills", () => {
    ...
    cy.get(".nav-bar-text").last().click();
    cy.get(".search-title").contains("p", "Find people near you");
    cy.get(".react-autosuggest__input").should(
      "have.attr",
      "placeholder",
      "Click for suggestions"
    );
    cy.get(".react-autosuggest__input").click();
    cy.get(".react-autosuggest__suggestions-container--open").should(
      "have.length",
      1
    );
    ...
    cy.get("#react-autowhatever-1--item-0")
      .should("have.text", "juggling")
      .click();
    cy.get(".search-submit-btn").click();
    cy.wait("@searchJuggling");
    cy.get(".search-results-qty").contains(
      "h2",
      "Showing 1 Results for juggling"
    );
